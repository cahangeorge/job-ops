...
- For `openai`, JobOps defaults to `gpt-5.6-luna` when the global model field is blank; tailoring uses `gpt-5.6-terra` by default when no explicit global model override is configured.
