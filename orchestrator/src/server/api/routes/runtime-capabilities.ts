...
const capabilityIds = new Set(["pdf", "queue", "llm", "providers", "extractors"]);
...
function toRuntimeCapabilitiesDto(value: RuntimeCapabilityHealthResponse): RuntimeCapabilityHealthResponse {
  return {
    checkedAt: boundedText(value.checkedAt, new Date(0).toISOString()),
    capabilities: value.capabilities.flatMap((capability) => {
      if (!capabilityIds.has(capability.id) || !capabilityStates.has(capability.state)) {
        return [];
      }
      return [{
        id: capability.id,
        ...
        reason: boundedText(capability.reason, "No status detail is available."),
      }];
    }),
  };
}
