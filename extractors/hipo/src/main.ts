...
async function run(): Promise<void> {
  const outputPath =
    process.env.HIPO_OUTPUT_JSON ||
    join(__dirname, "../storage/datasets/default/jobs.json");

  const result = await runHipo({
    searchTerms: parseSearchTerms(
      process.env.HIPO_SEARCH_TERMS ?? process.env.HIRING_CAFE_SEARCH_TERMS,
      DEFAULT_SEARCH_TERM,
    ),
    maxJobsPerTerm: parsePositiveInt(
      process.env.HIPO_MAX_JOBS_PER_TERM ?? process.env.JOBSPY_RESULTS_WANTED,
  ...
  if (!result.success) {
    throw new Error(result.error ?? "Hipo extractor failed");
  }

  await mkdir(dirname(outputPath), { recursive: true });
  await writeFile(outputPath, `${JSON.stringify(result.jobs, null, 2)}\n`);
  console.log(`Hipo extractor wrote ${result.jobs.length} jobs`);
}

run().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : "Unknown error";
  console.error(`Hipo extractor failed: ${message}`);
  process.exitCode = 1;
});