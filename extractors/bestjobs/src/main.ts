...
import { runBestJobs } from "./run.js";
...
async function run(): Promise<void> {
  const outputPath =
    process.env.BESTJOBS_OUTPUT_JSON ||
    join(__dirname, "../storage/datasets/default/jobs.json");

  const result = await runBestJobs({
    searchTerms: parseSearchTerms(
      process.env.BESTJOBS_SEARCH_TERMS ?? process.env.HIRING_CAFE_SEARCH_TERMS,
      DEFAULT_SEARCH_TERM,
    ),
    maxJobsPerTerm: parsePositiveInt(
      process.env.BESTJOBS_MAX_JOBS_PER_TERM ?? process.env.JOBSPY_RESULTS_WANTED,
  ...
  if (!result.success) {
    throw new Error(result.error ?? "BestJobs extractor failed");
  }

  await mkdir(dirname(outputPath), { recursive: true });
  await writeFile(outputPath, `${JSON.stringify(result.jobs, null, 2)}\n`);
  console.log(`BestJobs extractor wrote ${result.jobs.length} jobs`);
}

run().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : "Unknown error";
  console.error(`BestJobs extractor failed: ${message}`);
  process.exitCode = 1;
});