...
import { runEJobs } from "./src/run";

type EJobsContext = Parameters<ExtractorManifest["run"]>[0];
...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `eJobs: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  ...
  if (event.type === "page_complete") {
    return {
      ...
      detail: `eJobs: term ${event.termIndex}/${event.termTotal}, page ${event.page} (${event.totalCollected} collected)`,
    };
  }
  ...
  return {
    ...
    detail: `eJobs: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`,
  };
}
...
export const manifest: ExtractorManifest = {
  id: "ejobs-extractor",
  displayName: "eJobs",
  providesSources: ["ejobs"],
  async run(context: EJobsContext) {
